...
class SpecialUserProvisionerTest {
    ...
    private static final User MEMBER = new User(MEMBER_USER_ID, ORGANIZATION_ID, "display name", "user@example.com");
    ...
    void setUp() {
        ...
        when(keycloakSynchronizationService.setupKeycloakUser(MEMBER.getEmailAddress(), true, ORGANIZATION_ID,
            Set.of(ORG_USER),
    ...
    void provision_WillCreateUserInKeycloak() {
        when(usersRepository.findByEmailAddressIgnoreCase(MEMBER.getEmailAddress())).thenReturn(Optional.empty());

        specialUserProvisioner.provision(MEMBER, "password", Set.of(ORG_USER));

        verify(keycloakSynchronizationService).setupKeycloakUser(MEMBER.getEmailAddress(), true,
    ...
    void provision_WillCreateUserInLocalRepository() {
        ...
        verify(usersRepository).save(
            new PersistableUser(MEMBER_USER_ID, ORGANIZATION_ID, MEMBER.getDisplayName(), MEMBER.getEmailAddress(), false,
                Instant.now(clock)));
    }
    ...
    void provision_WillSkipCreatingAdminUser_WhenAdminUserAlreadyExists() {
        when(usersRepository.findByEmailAddressIgnoreCase(MEMBER.getEmailAddress())).thenReturn(Optional.of(mock(PersistableUser.class)));
